...
func (p *Postgres) ResetStats(chatID int64) error {
	return p.db.Table("user_in_chats").
...
func (p *Postgres) GetUserStats(userID int, chatID int64) (model.UserInChat, model.UserInChat, error) {
    var (
        forCurrentChat model.UserInChat
        forAllChats    model.UserInChat
        err            error
    )
    ...
    err = p.db.
        ...
        Where("id = ? AND chat_id = ?", userID, chatID).
        First(&forCurrentChat).
        Error
    ...
    err = p.db.
        ...
        Select(`
            id,
            0                      AS chat_id,
            sum(was_host)          AS was_host,
            sum(success)           AS success,
            sum(guessed)           AS guessed,
    ...
    return forCurrentChat, forAllChats, err
}
